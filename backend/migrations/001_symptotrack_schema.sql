-- ============================================================
-- SymptoTrack PRD v1.0 - Database Migration
-- From: Healthcare Management Platform v1.0
-- To: SymptoTrack-compliant schema
-- Date: 2026-02-26
-- ============================================================
-- This migration adds 18 new tables and extends 5 existing tables
-- to support SymptoTrack PRD requirements.
-- ============================================================
...
-- Make password_hash nullable (OTP-only login)
...
-- ============================================================
-- 2. NEW TABLES — Tests Ordered (needed before medical_reports FK)
-- ============================================================
...
-- Medical Reports: Link to prescription and test order
ALTER TABLE medical_reports ADD COLUMN IF NOT EXISTS prescription_id UUID REFERENCES prescriptions(id) ON DELETE SET NULL;
ALTER TABLE medical_reports ADD COLUMN IF NOT EXISTS test_order_id UUID REFERENCES tests_ordered(id) ON DELETE SET NULL;
ALTER TABLE medical_reports ADD COLUMN IF NOT EXISTS follow_up_date DATE;
ALTER TABLE medical_reports ADD COLUMN IF NOT EXISTS structured_values JSONB;
...
CREATE TABLE IF NOT EXISTS test_reminders (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    prescription_id UUID NOT NULL REFERENCES prescriptions(id) ON DELETE CASCADE,
    test_order_id UUID NOT NULL REFERENCES tests_ordered(id) ON DELETE CASCADE,
    patient_id UUID NOT NULL REFERENCES patients(id) ON DELETE CASCADE,
    test_name VARCHAR(200) NOT NULL,
    follow_up_date DATE,
...
-- Medical Reports
CREATE INDEX IF NOT EXISTS idx_medical_reports_prescription ON medical_reports(prescription_id);
...
CREATE TRIGGER update_tests_ordered_updated_at BEFORE UPDATE ON tests_ordered
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_digital_signatures_updated_at BEFORE UPDATE ON digital_signatures
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_test_reminders_updated_at BEFORE UPDATE ON test_reminders
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_doctor_verifications_updated_at BEFORE UPDATE ON doctor_verifications
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_doctor_medicine_favorites_updated_at BEFORE UPDATE ON doctor_medicine_favorites
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
